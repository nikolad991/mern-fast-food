...
// https://d33wubrfki0l68.cloudfront.net/9136862b7ab7459042ba826f6e84a312f6e0ef0a/32efc/trikolori-pizza.webp

const FeaturedProducts = () => {
  const [featuredProducts, setFeaturedProducts] = useState([]);
  useEffect(() => {
    fetch("http://localhost:4000/api/products/featured").then((res) =>
        ...
        {featuredProducts.map((product, index) => (
          <div key={index} className="flex flex-col items-center gap-6 p-4">
            ...
            <div className="bg-red-300 w-80 p-4 rounded-md flex flex-col gap-2 text-center">
              ...
              <Link to={`/product/${product._id}`} className="bg-slate-800 rounded-lg text-white w-fit mx-auto px-3 py-1">
                Order
              </Link>
            </div>
          </div>
